...
async def download_media(task: TriggerModerationTask) -> bytes | None:
    """Download media and return raw image bytes."""
    ...
    if is_video:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            if not await download_file_to_path(file_url, str(video_path)):
                return None
            return await extract_frame_from_video_path(video_path, position=0.5)
    else:
...
async def handle_moderation_result(
    ...
    if not result:
        ...
        await add_history_step(
            session, trigger_id, ModerationStep.AUTO_ERROR, details={"error": "AI failed to process"}
        )
        ...
        if await session.get(Trigger, trigger_id):
            alert = ModerationAlert(
                trigger_id=trigger_id, chat_id=chat_id, category="Error", reasoning="AI failed to process"
            )
    ...
    if result.category == "Safe":
        ...
        await add_history_step(
            session, trigger_id, ModerationStep.AUTO_APPROVED, details={"reasoning": result.reasoning}
        )
        await session.commit()
        await valkey.delete(f"triggers:{chat_id}")
        logger.info(f"Trigger {trigger_id} Safe ({result.source}): {result.reasoning}")
    else:
        trigger.moderation_status = ModerationStatus.FLAGGED
        ...
        if await session.get(Trigger, trigger_id):
            ...
            await add_history_step(session, trigger_id, ModerationStep.ALERT_SENT)
            await session.commit()
            logger.info(f"Trigger {trigger_id} flagged: {result.category} ({result.source})")

...
const ChatDetails: React.FC = () => {
  ...
  const unbanChat = async () => {
    if (!id) return;

    const confirmed = await confirm({
      title: 'Снять бан',
      message: 'Чат снова сможет пользоваться ботом, а его триггеры вернутся в общий список. Продолжить?',
    ...
    try {
      await chatsApi.unban(parseInt(id));
      const chatData = await chatsApi.getById(parseInt(id));
      setChat(chatData);
        ...
        <div>
          <Card icon={AlertTriangle} title="Действия">
            <div className="flex flex-wrap gap-3">
              <button
                onClick={banChat}
                className="flex-1 min-w-[130px] bg-danger text-white p-3 rounded-lg font-bold border-none cursor-pointer"
              >
              ...
              {chat.is_banned && (
                <button
                  onClick={unbanChat}
                  className="flex-1 min-w-[130px] bg-elevated text-text p-3 rounded-lg font-bold border-none cursor-pointer"
                >
                  Снять бан
                </button>
              )}
